...
- **Pi 完全静默**：Pi 不向任何 AstrBot 会话发送内容；任务进入 completed、failed、cancelled 或 orphaned 后，插件只创建 AstrBot 原生 active-agent 唤醒事件，由主模型自己读取 Pi session 并决定是否回复或发送文件。终态唤醒禁止转发 Pi 原文、JSONL、工具日志和内部状态，只允许发送整理后的用户可见回复。
